...
export const claudeCodeAdapter: ClientAdapter = {
  ...
  async detect() {
    try {
      await fs.access(getClaudeCodeConfigPath());
      return true;
    } catch {
      return false;
    }
  ...
  async readConfig() {
    try {
      const content = await fs.readFile(getClaudeCodeConfigPath(), 'utf8');
  ...
  canonicalToSchema(canonical: CanonicalConfig, currentSchema: unknown) {
    ...
    for (const [name, srv] of Object.entries(canonical.mcpServers)) {
      if (
        ...
        srv.enabledFor.includes('claude-code')
      ) {
